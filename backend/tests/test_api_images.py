...
ISSUE_ID = sample_issue_id(A_NEWSPAPER)
...
def test_the_cached_images_are_served(catalogue_client: TestClient, name: str, width: int) -> None:
    response = catalogue_client.get(f"/api/issues/{ISSUE_ID}/{name}.jpg")

    assert response.status_code == 200
    assert response.headers["content-type"] == "image/jpeg"
    assert response.headers["cache-control"] == "no-cache"
    assert response.headers["etag"].startswith(f'"{ISSUE_ID}-{name}-')
    assert response.headers["etag"].endswith('"')
    assert image_of(response.content).width == width


def test_a_versioned_url_is_immutable(catalogue_client: TestClient) -> None:
    issue = catalogue_client.get(f"/api/issues/{ISSUE_ID}").json()
...
@pytest.mark.parametrize(
    "path",
    [
        f"/api/issues/{ISSUE_ID}/cover.jpg",
        f"/api/issues/{ISSUE_ID}/thumb.jpg",
        f"/api/issues/{ISSUE_ID}/pages/1.webp",
    ],
)
def test_head_answers_an_image_with_its_headers_and_no_body(
    catalogue_client: TestClient, path: str
) -> None:
...
def test_a_matching_validator_is_a_304(catalogue_client: TestClient) -> None:
    etag = catalogue_client.get(f"/api/issues/{ISSUE_ID}/thumb.jpg").headers["etag"]

    response = catalogue_client.get(
        f"/api/issues/{ISSUE_ID}/thumb.jpg", headers={"If-None-Match": etag}
    )
...
def test_a_lost_cover_is_rendered_on_demand(
    ...
    cover, thumb = cover_paths(catalogue_settings.cache_path, ISSUE_ID)
    cover.unlink()
    thumb.unlink()

    response = catalogue_client.get(f"/api/issues/{ISSUE_ID}/cover.jpg")
...
def test_a_page_is_rendered_at_the_width_asked_for(
    ...
    response = catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/1.webp", params={"w": 800})

    assert response.status_code == 200
    assert response.headers["content-type"] == "image/webp"
    assert response.headers["cache-control"] == "no-cache"  # no ?v= on this URL
    assert response.headers["etag"].startswith(f'"{ISSUE_ID}-')
    assert response.headers["etag"].endswith('-1-800"')
    image = image_of(response.content)
    assert image.format == "WEBP"
    assert image.width == 800
    assert (page_cache_dir(catalogue_settings.cache_path, ISSUE_ID) / "1-800.webp").is_file()


def test_the_second_request_never_touches_the_renderer(
    catalogue_client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    first = catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/2.webp", params={"w": 400})
    ...
    second = catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/2.webp", params={"w": 400})
    ...
    with pytest.raises(AssertionError, match="rendered again"):
        catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/2.webp", params={"w": 1600})


@pytest.mark.parametrize(("asked", "served"), [(999, 1200), (5000, 2000), (1, 200), (800, 800)])
def test_a_width_is_snapped_to_the_ladder(
    catalogue_client: TestClient, asked: int, served: int
) -> None:
    response = catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/1.webp", params={"w": asked})
...
def test_no_width_means_the_default(catalogue_client: TestClient) -> None:
    response = catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/1.webp")

    assert image_of(response.content).width == DEFAULT_WIDTH


@pytest.mark.parametrize("width", [0, -1, 10**9])
def test_a_nonsense_width_is_refused(catalogue_client: TestClient, width: int) -> None:
    response = catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/1.webp", params={"w": width})

    assert response.status_code == 422


def test_a_page_outside_the_document_is_a_404(catalogue_client: TestClient) -> None:
    page_count = catalogue_client.get(f"/api/issues/{ISSUE_ID}").json()["page_count"]

    assert catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/0.webp").status_code == 404
    assert (
        catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/{page_count + 1}.webp").status_code
        == 404
    )
    assert (
        catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/{page_count}.webp").status_code == 200
    )
...
def test_a_cache_hit_records_the_access(
    ...
    catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/1.webp", params={"w": 200})
    cached = page_cache_dir(catalogue_settings.cache_path, ISSUE_ID) / "1-200.webp"
    os.utime(cached, (0, 0))
...
def test_the_page_template_carries_the_version(catalogue_client: TestClient) -> None:
    ...
    assert template == f"/api/issues/{ISSUE_ID}/pages/{{n}}.webp?w={{w}}&v={version}"
...
def test_a_stale_version_is_served_but_never_immutable(catalogue_client: TestClient) -> None:
    """A replaced PDF keeps its id and its URLs; the version is what moves."""
    current = catalogue_client.get(
        f"/api/issues/{ISSUE_ID}/pages/1.webp", params={"w": 400, "v": _version(catalogue_client)}
    )
    stale = catalogue_client.get(
        f"/api/issues/{ISSUE_ID}/pages/1.webp", params={"w": 400, "v": "1"}
    )
    missing = catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/1.webp", params={"w": 400})
...
def test_a_covers_version_is_checked_too(catalogue_client: TestClient) -> None:
    matching = catalogue_client.get(
        f"/api/issues/{ISSUE_ID}/cover.jpg", params={"v": _version(catalogue_client)}
    )
    stale = catalogue_client.get(f"/api/issues/{ISSUE_ID}/cover.jpg", params={"v": "1"})

    assert matching.headers["cache-control"] == "public, max-age=31536000, immutable"
    assert stale.headers["cache-control"] == "no-cache"


def test_the_page_validator_moves_with_the_document(catalogue_client: TestClient) -> None:
    etag = catalogue_client.get(f"/api/issues/{ISSUE_ID}/pages/1.webp", params={"w": 400}).headers[
        "etag"
    ]

    assert _version(catalogue_client) in etag
...
def test_two_requests_for_a_cold_cover_render_it_once(
    catalogue_client: TestClient, catalogue_settings: Settings, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        responses = list(
            pool.map(
                ...
                [f"/api/issues/{ISSUE_ID}/cover.jpg", f"/api/issues/{ISSUE_ID}/thumb.jpg"],
            )
        )
...
def test_a_cover_is_written_atomically(catalogue_settings: Settings) -> None:
    """The path holds a whole image or the previous one, never half of one."""
    cover, _ = cover_paths(catalogue_settings.cache_path, ISSUE_ID)
    ...
    try:
        for _ in range(5):
            render_cover(
                catalogue_settings.library / A_NEWSPAPER, ISSUE_ID, catalogue_settings.cache_path
            )
    finally:
        stop.set()
...
def test_a_sweep_never_takes_the_page_it_was_triggered_by(
    ...
    for page in (1, 2, 3):
        response = catalogue_client.get(
            f"/api/issues/{ISSUE_ID}/pages/{page}.webp", params={"w": 800}
        )
